function [] = plotanova_rm_closerfarther(ds, prop)
% plotanova_rm_closerfarther(ds, prop)
% 
...
% trick to sort the labels for the Closer_stimulus_... fields
    a=getlabels(labels); b=[];
...
for i=1:length(ds)
    if isempty(ds{i})~=1
        ...
        for j=1:length(labels)
            mn(i,j)=mean(a(groups==nominal(labels(j))));
            sd(i,j)=std(a(groups==nominal(labels(j))));
            sm(i,j)=std(a(groups==nominal(labels(j))))/sqrt(size(a(groups==nominal(labels(j))),1));
        end
        ...
        for j=1:length(labels)
            ergroups(i,j)=mean(errors(groups==nominal(labels(j))));
        end
    end
end
...
subplot(1,3,1); errorbar(mean_mn,mean_sm,'*-'); xlabel(['F(',num2str(b{2,3}),',',num2str(b{4,3}),') = ',num2str(b{2,5}),', PV',PV]); ylabel('mean RT');
