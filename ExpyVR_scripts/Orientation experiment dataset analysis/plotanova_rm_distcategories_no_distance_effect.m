function [] = plotanova_rm_distcategories_no_distance_effect(ds, prop, num_categories)
...
% THIS SCRIPT IS FOR ANALYZING USING THE DISTANCE CATEGORIES - USUALLY 3 OR
% 4 CATEGORIES (FOR FMRI EXPERIMENTS)

% THE SCRIPT ONLY ANALYZES DISTANCES OF 0 BETWEEN CATEGORIES 

ds_no_err=ds; mn=ones(length(ds),num_categories)*(-1); sd=ones(length(ds),num_categories)*(-1); ergroups=ones(length(ds),num_categories)*(-1); sm=ones(length(ds),num_categories)*(-1);
for i=1:length(ds)
    if isempty(ds{i})~=1
        % removing errors
        ...
        if strcmp(prop,'Closest_dist_user') || strcmp(prop,'Farthest_dist_user') || strcmp(prop,'Between_dist_user')
            %ds_no_err{i}.(prop) = round(ds_no_err{i}.(prop)*(10/max(ds_no_err{i}.(prop))));
            qc=ds_no_err{i}.Closest_dist_user;
            qf=ds_no_err{i}.Farthest_dist_user;
            for j=1:num_categories
                qc(qc>(10/num_categories*(j-1)) & qc<=10/num_categories*j) = j;
                qf(qf>(10/num_categories*(j-1)) & qf<=10/num_categories*j) = j;                
            end
            ds_no_err{i}.Closest_dist_user=qc;
            ds_no_err{i}.Farthest_dist_user=qf;
            ds_no_err{i}.Between_dist_user=abs(qc-qf)+1;
        
            % removing distances larger than 0 - comparing only 1-1, 2-2, 3-3
            % (or more depending on num_categories) to each other    
            ds_no_err{i}(ds_no_err{i}.Between_dist_user~=1,:)=[];
        end
        ...
        for j=1:num_categories
            mn(i,j)=mean(a(groups==j));
            sd(i,j)=std(a(groups==j));
            sm(i,j)=std(a(groups==j))/sqrt(size(a(groups==j),1));
        end
        ...
        for j=1:num_categories
            ergroups(i,j)=mean(errors(groups==j));
        end
    end
end
size(mn)
