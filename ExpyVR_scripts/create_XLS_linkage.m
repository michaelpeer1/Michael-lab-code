function create_XLS_linkage(filenames, TR, len_stimulus, len_end_rest, num_excessive_TRs, num_removed_volumes)
% create_XLS_linkage(filenames, TR, len_stimulus, len_end_rest, num_excessive_TRs, num_removed_volumes)
%
% This function is similar to create_ER_design_XLS_w_quest_fields.m, but
% simpler, for analysis of the results of the linkage-analysis distance
% project
%
% receives:
...
for q=1:length(filenames)
    ...
    times=round(times*2)/2+cur_start_time;
    ...
    data{q} = textscan(a, '%f %f %f %f %s', 'Delimiter',',','HeaderLines',1);
    fclose(a);
    a=fopen(strcat(LOGPATH,filenames{q},'_output.csv'),'r');                                            % the output log file
    header=textscan(a, '%s %s %s %s %s %s %s', 'Delimiter',',');
    fclose(a);
    stimulus_filename=header{7}{1};
...
%     end
    ...
    % removing the first (false) condition in the output file
    for i=1:length(data{q})
        data{q}{i} = data{q}{i}(2:end);
    end
    data{q}{1}=data{q}{1}-1;
    ...
    for i=1:length(names)-1
        for j=1:length(onsets{i})
            new_data{end+1,1}=onsets{i}(j)*TR;
...
%         new_data{data{q}{1}(i)+1,curr_col}=data{q}{3}(i);    % index_R
%         new_data{data{q}{1}(i)+1,curr_col+1}=data{q}{4}(i);    % index_L
    end
...
%     if length(filenames)==1
        xlswrite(strcat(LOGPATH,filenames{q},'_XLS_for_distances.xls'),[headers;all_new_data]);
%     else
