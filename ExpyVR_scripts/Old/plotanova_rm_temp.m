function [] = plotanova_rm_temp(ds, prop)
...
for i=1:length(ds)
    if isempty(ds{i})~=1
        ...
        % removing conditions
        conds=unique(ds_no_err{i}.condition);
        ds_no_err{i}(ds_no_err{i}.condition==conds(1),:)=[];
        ds_no_err{i}(ds_no_err{i}.condition==conds(2),:)=[];
        %ds_no_err{i}(ds_no_err{i}.condition==conds(3),:)=[];
