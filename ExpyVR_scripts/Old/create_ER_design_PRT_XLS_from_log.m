function [names, onsets, durations]=create_ER_design_PRT_XLS_from_log(filenames, times_start_TRs, TR, len_stimulus, len_end_rest, num_excessive_TRs, num_removed_volumes)
% receives:
...
%             durations{end}=[durations{end} LEN_REST/TR];            
    ...
    durations{end}=[durations{end} len_end_rest/TR];
...
m_new(:,end-1)=num2cell(onsets{end}+1);
m_new(:,end)=num2cell(onsets{end}+durations{end});
names_new_new=cell(1,length(names)*2); names_new_new(1:2:end)=names;
xlswrite([LOGPATH filenames{1} '_PRT_ER_design.xls'],[names_new_new;m_new]);
...
xlswrite([LOGPATH filenames{1} '_PRT_ER_domain_design.xls'],[names_new_new;m_new]);
