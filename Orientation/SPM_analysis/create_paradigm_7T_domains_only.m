function create_paradigm_7T_domains_only(functional_dirname, subj_name, paradigm_filenames, add_mov_regressors, ignore_sessions, time_derivative, dispersion_derivative, output_dir)
...
for i=1:numsess
    ...
    matlabbatch{1}.stats{1}.fmri_spec.sess(i).multi = cellstr([expyvr_log_dir '\' paradigm_filenames{i} '_domain_design.mat']);
...
i=11; contypes{i}=1; contrast_names{i}= 'Person vs. time'; basic_contrast_vectors{i}=[1 0 -1];basic_contrast_vector_control{i}=zeros(1,3);
i=12; contypes{i}=1; contrast_names{i}= 'Place vs. person'; basic_contrast_vectors{i}=[-1 1 0];basic_contrast_vector_control{i}=zeros(1,3);
i=13; contypes{i}=1; contrast_names{i}= 'Place vs. time'; basic_contrast_vectors{i}=[0 1 -1];basic_contrast_vector_control{i}=zeros(1,3);
i=14; contypes{i}=1; contrast_names{i}= 'Time vs. person'; basic_contrast_vectors{i}=[-1 0 1];basic_contrast_vector_control{i}=zeros(1,3);
i=15; contypes{i}=1; contrast_names{i}= 'Time vs place'; basic_contrast_vectors{i}=[0 -1 1];basic_contrast_vector_control{i}=zeros(1,3);
...
i=16; contypes{i}=1; contrast_names{i}= 'Domains vs control'; basic_contrast_vectors{i}=ones(1,3); basic_contrast_vector_control{i}=ones(1,3)*(-1)*(numsess-1);
i=17; contypes{i}=1; contrast_names{i}= 'Person vs control'; basic_contrast_vectors{i}=[1 0 0]; basic_contrast_vector_control{i}=ones(1,3)*(-1)*(numsess-1)/3;
i=18; contypes{i}=1; contrast_names{i}= 'Place vs control'; basic_contrast_vectors{i}=[0 1 0]; basic_contrast_vector_control{i}=ones(1,3)*(-1)*(numsess-1)/3;
i=19; contypes{i}=1; contrast_names{i}= 'Time vs control'; basic_contrast_vectors{i}=[0 0 1]; basic_contrast_vector_control{i}=ones(1,3)*(-1)*(numsess-1)/3;
