load('C:\Subjects_MRI_data\7T\121119_Chrystany\Analysis\GLM_normalized_domains_only\SPM.mat');
%load('C:\Subjects_MRI_data\7T\2nd-level\New\matlabbatch.mat')

cons=SPM.xCon;
for i=1:length(cons)
    % creating the new directory
    dirname=['C:\Subjects_MRI_data\7T\2nd-level\New_domains_only\' num2str(i) '_' cons(i).name];
    mkdir(dirname);
    ...
    for j=1:length(subjnames)
        scans{end+1}=['C:\Subjects_MRI_data\7T\' subjnames{j} '\Analysis\GLM_normalized_domains_only\' contrast_filename];
    end
