...
% experiment 7-Tesla subjects - only the new subjects from 2013
%
...
output_dir='E:\Subjects_MRI_data\7T\New_subjs_prep_2013';
a=dir([path '13*']); num_subjs=length(a);

% pre-processing with script
parfor i=1:num_subjs
    disp(i)
    b=dir([path a(i).name '\Nifti\*distance*']); NumSessions=length(b);
    func_dirnames=cell(1,NumSessions);
    for j=1:NumSessions
    ...
    % running the script
    pre_process_7T_subject_2013(func_dirnames, T1_uni_den_dirname{1}, func_wholebrain_dir{1}, output_dir, subject_name, start_from_stage, end_stage);
end
