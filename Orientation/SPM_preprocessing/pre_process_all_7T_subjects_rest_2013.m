% This is a script for pre-processing of the resting-state scans of the 
% orientation experiment 7-Tesla subjects - only the new subjects from
% 2013
%
% Runs the pre_process_7T_subject_rest_2013.m script on all subjects, and then
...
path='e:\Subjects_MRI_data\7T\Data\Distance_2013\';
output_dir='e:\Subjects_MRI_data\7T\New_subjs_prep_2013\Resting state';
a=dir([path '13*']); num_subjs=length(a);
...
parfor i=1:num_subjs
    ...
    T1_uni_den_dirname=getfullfiles([path a(i).name '\Nifti\*UNI-DEN*']);
    func_wholebrain_dir=getfullfiles([path a(i).name '\Nifti\*wholebrain*']);
    subject_name=a(i).name;
    % running the script
    pre_process_7T_subject_rest_2013(func_dirname, T1_uni_den_dirname{1}, func_wholebrain_dir{1}, output_dir, subject_name, start_from_stage, end_stage);
end
