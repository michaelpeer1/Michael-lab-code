...
%% making emotion based PRTs and SDMs
...
for s=1:num_subjects_to_use
    ...
    for i=1:length(prt_files)
        ...
        for linenum=2:size(curr_data_for_PRT,1)
            curr_emotion=curr_data_for_PRT{linenum,13};       % emotion is in column 13
            curr_domain=curr_data_for_PRT{linenum,2}(1:2);
            curr_trial_time=curr_data_for_PRT{linenum,1}+1;
            if ~isnan(curr_emotion)
                % add to adequate domain condition
                if strcmp(curr_domain,'pe')
                    condition_onoffsets{1,1}=[condition_onoffsets{1,1}; curr_trial_time curr_trial_time curr_emotion];
                elseif strcmp(curr_domain,'pl')
                    condition_onoffsets{1,2}=[condition_onoffsets{1,2}; curr_trial_time curr_trial_time curr_emotion];
                elseif strcmp(curr_domain,'ti')
                    condition_onoffsets{1,3}=[condition_onoffsets{1,3}; curr_trial_time curr_trial_time curr_emotion];
                end
                condition_onoffsets_all=[condition_onoffsets_all; curr_trial_time curr_trial_time curr_emotion];
            else
                if strcmp(curr_domain,'pe')
        ...
        prt_xff.SaveAs(fullfile(ACPC_output_dir,['paradigm_emotion_' num2str(i) '.prt']));
        ...
        sdm_inc_confounds.SaveAs(fullfile(ACPC_output_dir, [subj '_emotion_' num2str(i) '.sdm']));
...
%         prt_xff.SaveAs(fullfile(ACPC_output_dir,['paradigm_emotion_nodomains_' num2str(i) '.prt']));
...
%     motion_sdm_files(cellfun(@(x) ~isempty(strfind(x,'rest')), motion_sdm_files))=[];
    ...
    prt_files=getfullfiles(fullfile(ACPC_output_dir,'paradigm_emotion_*.prt'));
...
%         SDM_name = fullfile(ACPC_output_dir, [subj '_emotion_' num2str(i) '.sdm']);
...
%     vmr.AddStudyAndDesignMatrix(vtc_filename, SDM_name);
    ...
    % creating the MDM
    MDM_name = fullfile(ACPC_output_dir, [subj '_emotion_ACPC.mdm']);
    vmr.SaveMultiStudyGLMDefinitionFile(MDM_name);
    
    % creating GLM
    GLM_name = fullfile(ACPC_output_dir, [subj '_emotion_ACPC.glm']);
    ...
    CTR_name = fullfile(ACPC_output_dir, [subj '_emotion_ACPC.ctr']);
    ctr={}; ctr_names={};
    i=1; ctr_names{i}='emotion person effect'; ctr{i}=[repmat([0 1 0 0 0 0],1,num_sessions)];
    i=2; ctr_names{i}='emotion place effect'; ctr{i}=[repmat([0 0 0 1 0 0],1,num_sessions)];
    i=3; ctr_names{i}='emotion time effect'; ctr{i}=[repmat([0 0 0 0 0 1],1,num_sessions)];
    i=4; ctr_names{i}='emotion all domains effect'; ctr{i}=[repmat([0 1 0 1 0 1],1,num_sessions)];
