...
for s=1:num_subjects_to_use
    ...
    for i=1:length(prt_files)
        ...
        for linenum=2:size(curr_data_for_PRT,1)
            curr_RT=curr_data_for_PRT{linenum,7};       % response time is in column 7
            ...
            if ~isnan(curr_RT)
                ...
                if strcmp(curr_domain,'pe')
                    condition_onoffsets{1,1}=[condition_onoffsets{1,1}; curr_trial_time curr_trial_time curr_RT];
                elseif strcmp(curr_domain,'pl')
                    condition_onoffsets{1,2}=[condition_onoffsets{1,2}; curr_trial_time curr_trial_time curr_RT];
                elseif strcmp(curr_domain,'ti')
                    condition_onoffsets{1,3}=[condition_onoffsets{1,3}; curr_trial_time curr_trial_time curr_RT];
                end
                condition_onoffsets_all=[condition_onoffsets_all; curr_trial_time curr_trial_time curr_RT];
            else
                if strcmp(curr_domain,'pe')
        ...
        sdm_inc_confounds.SaveAs(fullfile(ACPC_output_dir, [subj '_RT_' num2str(i) '.sdm']));
...
%         prt_xff.SaveAs(fullfile(ACPC_output_dir,['paradigm_RT_nodomains_' num2str(i) '.prt']));
...
%     vmr.AddStudyAndDesignMatrix(vtc_filename, SDM_name);
    ...
    ctr={}; ctr_names={};
    i=1; ctr_names{i}='RT person effect'; ctr{i}=[repmat([0 1 0 0 0 0],1,num_sessions)];
    i=2; ctr_names{i}='RT place effect'; ctr{i}=[repmat([0 0 0 1 0 0],1,num_sessions)];
    i=3; ctr_names{i}='RT time effect'; ctr{i}=[repmat([0 0 0 0 0 1],1,num_sessions)];
    i=4; ctr_names{i}='RT all domains effect'; ctr{i}=[repmat([0 1 0 1 0 1],1,num_sessions)];
